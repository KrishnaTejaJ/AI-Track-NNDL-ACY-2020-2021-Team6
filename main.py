...
parser.add_argument('--model', type=str, default=None, help='model to select')
# --- CNNRNN option
...
#headers=['window','horizon','hidRNN','dropout','residual_window','ratio']
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        #print('end of epoch {:3d} | time: {:5.2f}s | train_loss {:5.8f} | valid rse {:5.4f}'.format(epoch, (time.time() - epoch_start_time), train_loss, val_loss))
    #print('| end of a combination | time: {:5.2f}s | train_loss {:5.8f} | valid rse {:5.4f}'.format((time.time() - epoch_start_time), train_loss, val_loss))
    model_path = '%s/%s.pt' % (args.save_dir, args.save_name)
    with open(model_path, 'wb') as f:
      torch.save(model.state_dict(), f)
    with open(model_path, 'rb') as f:
      model.load_state_dict(torch.load(f));
    test_acc = evaluate(Data, Data.test, model, evaluateL2, evaluateL1, args.batch_size);
    #print ("test rse {:5.4f}".format(test_acc))
    #print("----------------------------------------------------------------")
    #print(args.window,args.horizon,args.hidRNN,args.dropout,args.residual_window,args.ratio,train_loss,val_loss,test_acc)
    fields = [args.window,args.horizon,args.hidRNN,args.dropout,args.residual_window,args.ratio,train_loss,val_loss,test_acc]
